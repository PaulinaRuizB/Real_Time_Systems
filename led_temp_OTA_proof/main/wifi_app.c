...
// Contador de intentos fallidos de conexión al router.
// Si llega a MAX_CONNECTION_RETRIES, el ESP32 deja de intentar.
	...
	if (event_base == WIFI_EVENT)
	{
		switch (event_id)
		{
			...
			case WIFI_EVENT_STA_DISCONNECTED:
				...
				// Si no hemos superado el límite de reintentos, volvemos a intentar
				if (g_retry_number < MAX_CONNECTION_RETRIES)
				{
					esp_wifi_connect();
					g_retry_number++;
					ESP_LOGI(TAG, "Reintentando conexion... intento %d de %d",
							g_retry_number, MAX_CONNECTION_RETRIES);
				}
				else
				{
					// Ya agotamos los reintentos, avisamos a la tarea principal
					ESP_LOGI(TAG, "Conexion fallida tras %d intentos", MAX_CONNECTION_RETRIES);
	...
	else if (event_base == IP_EVENT)
	{
		switch (event_id)
		{
			case IP_EVENT_STA_GOT_IP:
				...
				// Conexión exitosa: reseteamos el contador y avisamos a la tarea
				g_retry_number = 0;
		...
		if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					// Aquí podrías encender un LED rojo, mostrar error en la web, etc.
					break;
			}
		}
	}
